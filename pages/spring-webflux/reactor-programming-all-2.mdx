...
Cold Publisher 기반의 식은 실행중인 스레드는 Cold Publisher 를 구독할 때에만 Cold Publisher 에 대해 연산(evaluate)을 하게 되는데, Hot Publisher 와 비교해보면, Hot Publisher 는 구독하기 전에 이미 연산(evaluate)을 즉시합니다. Hot Publisher 와 같이 즉시 연산을 하는 것을 Eager Evaluation 이라고 이야기합니다.<br/>
